...
/*   Updated: 2023/07/28 23:43:22 by iouazzan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void check_ex_cgi(std::string file, int sock_srv, int sock_clt)
{
    ...
    if (!ex.empty() && (ex.compare(".py") == 0 || ex.compare(".sh") == 0)) {
...
int f_cgi(int sock_srv, int sock_clt)
{
	...
	if (fr == 0) {
        std::string ex;
        std::string new_file = servs.at(sock_srv).clts.at(sock_clt).fd_name + "_new";
        ...
        int fd_out = open("hi", O_CREAT | O_RDWR, 0644);
        if (!fd_out) {
            std::cout << "sfsf\n";
        }
        // dup2(fd_in, 0);
